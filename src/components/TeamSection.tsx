    ...
    <section className="py-24 sm:py-32 bg-[#F9F9F6] relative border-t border-[#111111]/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
          ...
          <Link 
            to="/equipe" 
            className="group inline-flex items-center text-sm font-medium uppercase tracking-wide text-[#111111] hover:text-[#B6966B] transition-colors"
          >
          ...
          {partners.map((partner, index) => (
            <Link to="/equipe" key={index} className="group cursor-pointer">
              ...
              <div>
                ...
                <p className="text-[#111111]/60 font-medium tracking-wide uppercase text-xs">{partner.role}</p>
              </div>
            </Link>
          ))}
        </div>
